...
EMAIL_FEATURE_COMBINATIONS = [
    {"use_original": True, "use_degree": False, "use_centrality": False, "use_local": False},  # baseline
    {"use_original": True, "use_degree": True, "use_centrality": False, "use_local": False},
    {"use_original": True, "use_degree": True, "use_centrality": True, "use_local": False},
    {"use_original": True, "use_degree": True, "use_centrality": True, "use_local": True},
    {"use_original": False, "use_degree": True, "use_centrality": False, "use_local": False},
    {"use_original": False, "use_degree": True, "use_centrality": True, "use_local": False},
    {"use_original": False, "use_degree": True, "use_centrality": True, "use_local": True},
    {"use_original": False, "use_degree": False, "use_centrality": True, "use_local": True},
]
    ...
    for i in range(n_configs):
        ...
        try:
            ...
            if is_better:
                ...
                best_history, best_test_metrics, points = train_single_model_dataset_config(
                    model_name=model_name,
                    dataset_name=dataset_name,
                    hyperparams=config,
                    epochs=epochs,
...
def get_email_dataset_name(feature_config: Dict[str, bool]) -> str:
    """Generate a dataset name based on feature configuration."""
    parts = []
    if feature_config.get("use_original"):
        parts.append("orig")
    if feature_config.get("use_degree"):
        parts.append("deg")
    if feature_config.get("use_centrality"):
        parts.append("cent")
    if feature_config.get("use_local"):
        parts.append("local")
    if not parts:
        parts = ["none"]
    return "email_" + "_".join(parts)


def run_email_feature_ablation(args):
    """Run efficiency plots for all email feature combinations."""
    print(f"Running email feature ablation study...")
    print(f"Models: {args.models}")
    print(f"Feature combinations: {len(EMAIL_FEATURE_COMBINATIONS)}")

    results_all = {}
    for feature_config in EMAIL_FEATURE_COMBINATIONS:
        dataset_name = get_email_dataset_name(feature_config)
        print(f"\n{'='*60}")
        print(f"Feature config: {feature_config}")
        print(f"Dataset name: {dataset_name}")
        print(f"{'='*60}")

        # Create modified args with this feature config
        feature_args = argparse.Namespace(
            email_use_degree=feature_config.get("use_degree", False),
            email_use_centrality=feature_config.get("use_centrality", False),
            email_use_local=feature_config.get("use_local", False),
            email_use_original=feature_config.get("use_original", True),
        )
        # Copy over other args
        feature_args.data_dir = args.data_dir
        feature_args.models = args.models
        feature_args.hidden_channels = args.hidden_channels
        feature_args.num_layers = args.num_layers
        feature_args.dropout = args.dropout
        feature_args.norm = args.norm
        feature_args.lr = args.lr
        feature_args.weight_decay = args.weight_decay
        feature_args.gat_heads = args.gat_heads
        feature_args.K = args.K
        feature_args.alpha = args.alpha
        feature_args.n_hyperparam_trials = args.n_hyperparam_trials
        feature_args.early_stopping_patience = args.early_stopping_patience
        feature_args.epochs = args.epochs
        feature_args.device = args.device
        feature_args.verbose = args.verbose
    ...
    fig, axes = plt.subplots(3, 2, figsize=(15, 12))
    for dataset_name, data in results_all.items():
        for model_name, result in data["results"].items():
            ...
            for trial in tuning_results:
                ...
                label = f"{dataset_name}_{model_name}"
                # Training plots
                axes[0, 0].plot(steps, train_acc, label=label)
                axes[1, 0].plot(steps, train_f1, label=label)
                axes[2, 0].plot(steps, train_loss, label=label)

                # Validation plots
                axes[0, 1].plot(steps, val_acc, label=label)
                axes[1, 1].plot(steps, val_f1, label=label)
                axes[2, 1].plot(steps, val_loss, label=label)
    ...
    fig.suptitle("Email Feature Ablation Study", fontsize=16)

    output_dir = os.path.join("results", "efficiency_plots", "email_ablation")
    os.makedirs(output_dir, exist_ok=True)
    fig_path = os.path.join(output_dir, f"ablation_comparison.png")
    fig.savefig(fig_path)
    print(f"Saved ablation comparison plots to {fig_path}")
...
def main():
    ...
    parser.add_argument("--early-stopping-patience", type=int, default=20, help="Early stopping patience in epochs")
    ...
    run_email_feature_ablation(args)
